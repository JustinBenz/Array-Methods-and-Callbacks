...
function getWinnersByYear(cb1, cb2) {
    const countries = cb1(getFinals(fifaData));
    const year = cb2(getFinals(fifaData));
    const text = year.map((item, index) => {
       return `In ${item}, ${countries[index]} won the world cup!`
    });
    return text;
    };
...
function getAverageGoals(data) {
    let averageGoals = data.reduce(function(accumulator, element) {
        accumulator["Home"] += element["Home Team Goals"];
        accumulator["Away"] += element["Away Team Goals"];
        return accumulator;
    }, {Home: 0, Away: 0});
